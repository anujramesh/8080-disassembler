...
int disassemble(unsigned char* buffer, int pc) {
    ...
    // opbytes will be used to track the number of bytes followed by the opcode
    int opbytes = 1;
    printf("0x%04x\t", pc);

    switch (*opcode) {

        // 8080 instruction set: https://pastraiser.com/cpu/i8080/i8080_opcodes.html
        // Additional aid: http://www.emulator101.com/8080-by-opcode.html
        ...
        case 0xce:
            printf("ACI    #%02x%02x", opcode[2], opcode[1]);
            opbytes = 3;
            break;
        ...
        case 0xde:
            printf("SBI    $%02x%02x", opcode[2], opcode[1]);
            opbytes = 3;
            break;
        ...
        case 0xee:
            printf("XRI    $%02x%02x", opcode[2], opcode[1]);
            opbytes = 3;
            break;
